...
import { X, Loader2, Edit3, HelpCircle, Target } from 'lucide-react'
import { updateResearchRecord, ResearchType, ResearchStatus, ResearchPriority } from '@/lib/vault/actions'
...
export function ResearchEditModal({
  record,
  workspaceId,
  isOpen,
  onClose,
  ...
  const [researchType, setResearchType] = useState<ResearchType>(record.research_type || 'market')
  const [status, setStatus] = useState<ResearchStatus>(record.status || 'planning')
  const [priority, setPriority] = useState<ResearchPriority>(record.priority || 'medium')
  const [researchQuestion, setResearchQuestion] = useState(record.research_question || '')
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setError(err.message || 'Failed to update research record')
      }
    })
  }
